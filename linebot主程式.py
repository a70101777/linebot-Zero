...
#==============================================================================#
#cl = LINE('dateset012345@gmail.com', 'moumou001')
    ...
    try:
        ...
        if op.type == 5:
            if settings["autoAdd"] == True:
                ...
                sendMention(op.param1, " @! 感謝您加Threadripper™為好友",[op.param1])
        if op.type == 11:
...
#                    cl.kickoutFromGroup(to,[sender])
            ...
            if sender not in ban["blacklist"]:
                if text.lower() in ['妹子圖','抽妹子'] :
                    cl.sendImage(to, "linepy/loli/{}-monmon.png".format(str(random.randint(0,int(settings["monmonpic"]-1)))))
                elif text.lower() in ['loli','抽蘿莉'] :
                    cl.sendImage(to, "linepy/loli/{}-image.png".format(str(random.randint(0,int(settings["pic"]-1)))))
            if sender in ban["admin"] or sender in ban["owners"]:
                if text.lower() in ['speed','sp']:
                  cl.sendMessage(to,"測速中...\n結果約為"+str(timeit.timeit('"-".join(str(n) for n in range(100))',number=10000)) + "秒")
                elif text.lower() == '！其他設定':
                     setM = setm()
                     cl.sendMessage(to, str(setM))
                elif text.lower() == '！黑名單':
                     BlackM = Blackm()
                     cl.sendMessage(to, str(BlackM))
                elif text.lower() == '！邀請設定':
                     InvM = Invm()
                     cl.sendMessage(to, str(InvM))
                elif text.lower() == '！模仿設定':
                     MimicM = Mimicm()
                     cl.sendMessage(to, str(MimiM))
                elif text.lower() == '！破壞群組':
                     ExplosionM = Explosionm()
                     cl.sendMessage(to, str(ExplosionM))
                elif text.lower() == '！關於他人':
                     HimM = Himm()
                     cl.sendMessage(to, str(HimM))
                elif text.lower() == '！權限':
                     AdminM = Adminm()
                     cl.sendMessage(to, str(AdminM))
                elif text.lower() == '！廣播':
                     gdcM = gdcm()
                     cl.sendMessage(to, str(gdcM))
                elif text.lower() == '！關於群組':
                     groupM = groupm()
                     cl.sendMessage(to, str(groupM))
                elif text.lower() == '！群組設定':
                     groupsetM = groupsetm()
                     cl.sendMessage(to, str(groupsetM))
                elif text.lower() == '！公開指令':
                     MesM = Mesm()
                     cl.sendMessage(to, str(MesM))
                elif text.lower() == '!setting':
                     setM = setm()
                     cl.sendMessage(to, str(setM))
                elif text.lower() == '！系統設定':
                     settM = settm()
                     cl.sendMessage(to, str(settM))
                elif text.lower() == "boom!":
                    cl.sendMessage(to,"訊息炸彈")
                    cl.sendContact(to, "u6a6428fb35a5d362974d7bb2ed327724',")	
...
#==============================================================================#
            if sender in ban["owners"]:
                if text.lower() == 'restart':
                    ...
                    cl.sendMessage(to,"Threadripper™降臨撕裂")
                    cl.sendContact(to, "u6a6428fb35a5d362974d7bb2ed327724',")
                    if msg.toType == 2:
                        gs = cl.getGroup(msg.to)
                        for g in gs.members:
                            try:
                                ...
                                sleep(1)
                                cl.inviteIntoGroup(to, cl.findAndAddContactsByMid("u455b6f656dc76d989cf184b5604581f5"))
                            except:
                                pass
            ...
            if msg.contentType == 1:
                if wait["pic"] == True:
                    if msg._from in ban["owners"]:
                        image = cl.downloadObjectMsg(msg.id, saveAs="linepy/loli/{}-image.png".format(settings["pic"]))
                        settings["pic"] +=1
                        cl.sendMessage(msg.to,"OK")
                if wait["monmonpic"] == True:
                    if msg._from in ban["owners"]:
                        image = cl.downloadObjectMsg(msg.id, saveAs="linepy/loli/{}-monmon.png".format(settings["monmonpic"]))
